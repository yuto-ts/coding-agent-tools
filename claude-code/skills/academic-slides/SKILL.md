...
提出前に確認する。本文で述べた原則のうち、作業の終盤で崩れやすいものはここに再掲してある。

- [ ] 全ページで overflow なし(`check_layout.py`)
- [ ] 本編の充填率が 70〜95%
- [ ] 充填率は主文+補足の2段構成か図の拡大で上げた(文字を小さくして上げていない)
- [ ] 文字サイズが規定の段階数に収まっている
- [ ] 色と文字サイズは `deck.html` の CSS 変数にある値だけを使った
- [ ] 直近の修正は HTML 側で行い、PDF と pptx は生成し直した
- [ ] 背景・課題・関連研究に引用がある。番号はスライド登場順
- [ ] 全ページにページ番号(質疑でページを指定してもらうため)
- [ ] 1スライド1メッセージ。見出しがそのページの主張になっている
- [ ] 対比・分類の内容が箇条書きのままになっていないか(図解に置き換える)
- [ ] 制作の経緯に由来する記述が本編に残っていない(`ご指示により`、`今回は〜を扱わない` など)。聞き手に必要な範囲の宣言は残してよい
